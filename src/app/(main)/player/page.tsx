  ...
  return (
    <div className="relative -mx-8 -mt-4 flex min-h-[calc(100vh-90px)] flex-col overflow-hidden pb-12">
      ...
      <div className="relative z-10 flex items-center px-8 py-4">
        <Link href="/library" aria-label="Back" className="text-text-secondary hover:text-text-primary">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-xl flex-1 flex-col items-center justify-center px-8">
        <div className="h-[min(45vh,500px)] w-[min(45vh,500px)] shrink-0 overflow-hidden rounded-lg shadow-2xl">
          {bgUrl ? (
            <Image src={bgUrl} alt={currentTrack.title} width={500} height={500} className="h-full w-full object-cover" />
          ) : (
        ...
        <div className="mt-8 flex w-full items-start justify-between">
          <div className="min-w-0">
            <h1 className="truncate text-h1 text-text-primary">{currentTrack.title}</h1>
          ...
          >
            <Heart className={liked ? "h-6 w-6 fill-accent text-accent" : "h-6 w-6"} />
          </button>
        </div>

        <div className="mt-6 w-full">
          <SeekBar positionSec={positionSec} durationSec={currentTrack.durationSec} onSeek={seek} />
        </div>
